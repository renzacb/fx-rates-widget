...
export class SellFormComponent implements OnInit {
  ...
  async transact() {
    ...
    // ? Simulate selling transaction loading process
    this.utilitiesService.openSpinner();
    await new Promise((resolve) => setTimeout(() => resolve(null), 2000));
    this.utilitiesService.closeSpinner();

    const { fromAmount, toAmount, toCurrency } = this.forexForm.value;
    let gain = `${fromAmount} EUR`;
    let lost = `${toCurrency.currency} ${toAmount}`;
    if (this.currencyToBeSold === 'fromCurrency') {
      gain = `${toCurrency.currency} ${toAmount}`;
      lost = `${fromAmount} EUR`;
    }
    this.utilitiesService.openDialog({
      title: `SOLD ${lost}`,
